...
object Matches extends App:
  ...
  def process(p: Pair[Double,Pair[Double,Double]]): String = p match
    case Pair(1, Pair(1, 1)) => "A"    // matching a structure, defined
    case Pair(2, Pair(0, _)) => "B"    // matching a structure, partially specified
    ...
    case Pair(7, Pair(`z`,`z`)) => "F" + z // logical binding of `z`
    case Pair(8, OfLength(n)) => "G" + n  // usign a custom extractor
    case Pair(9, _: MyPair) => "H"  // usign a type check
    case _ => "?" // default case
